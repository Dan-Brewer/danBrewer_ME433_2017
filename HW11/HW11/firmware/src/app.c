...
void APP_Tasks(void) {
    ...
    switch (appData.state) {
        ...
        case APP_STATE_MOUSE_EMULATE:
        {
            ...
            ii = 0;
            for (ii; ii < 7; ii++) {
                dataP[ii] = datain[2 * ii + 1];
                dataP[ii] = (dataP[ii] << 8) | datain[2 * ii];
                dataP[ii] = dataP[ii]/(1500); //go from 0 - 10
            }
            // every 50th loop, or 20 times per second
            if (i > 10) {
                appData.mouseButton[0] = MOUSE_BUTTON_STATE_RELEASED;
                appData.mouseButton[1] = MOUSE_BUTTON_STATE_RELEASED;
                appData.xCoordinate = (int8_t) dataP[4]; //ax
                appData.yCoordinate = (int8_t) dataP[5]; //ay
                i = 0;
            }
            else{
                appData.mouseButton[0] = MOUSE_BUTTON_STATE_RELEASED;
