...
int duty = 899; //duty cycle for motors (1119 is max)
...
void OC1_init(void){
    //Timer3 and OC1 for Motor 2 PWM
    OSCCONbits.PBDIV = 0; //not sure what this does
    T3CONbits.TCKPS = 2; //prescaler N = 4
    PR3 = 1200 - 1; //for 1 kHz signal
    ...
    OC1RS = 0; //start at 0 duty cycle
    OC1R = 300; //Initialize before turning OC1 on (50%)
    OC3RS = 0;
    OC3R = 300;
...
void OC2_init(void){
    //Timer2 and OC2 for Servo PWM
    OSCCONbits.PBDIV = 0; //not sure what this does
    T2CONbits.TCKPS = 4; //Timer2 prescaler N=16
    PR2 = 60000 - 1; //for 50 Hz signal
    TMR2 = 0; //initial TMR2 count is 0
    OC2CONbits.OC32 = 0; //use 16 bit timer
    OC2CONbits.OCTSEL = 0; //use timer 2
    OC2CONbits.OCM = 0b110; //PWM mode w/o fault pin
    OC2RS = 4500; //should set motor to 90 degrees (1500 to 7500)
    OC2R = 4500; //Initialize before turning OC1 on (50%)
...
void APP_Initialize(void) {
    ...
    TRISBbits.TRISB2 = 0; //Set B2 to output for Motor 2 phase
    RPB3Rbits.RPB3R = 0b0101; //Set B3 to OC1 for Motor 2 enable
    RPB14Rbits.RPB14R = 0b0101; //Set B14 to OC3 for Motor 1 enable
    TRISBbits.TRISB15 = 0; //Set B15 to output for Motor 1 phase
    LATBbits.LATB15 = 0; //motor 1 runs forward
    LATBbits.LATB2 = 1; //motor 2 runs forward
    
    RPA1Rbits.RPA1R = 0b0101; //Set A1 to OC2
    
    OC1_init(); //initialize Motors PWM
    OC2_init(); //initialize Servo PWM
    OC1RS = duty/2; //50% duty
    OC3RS = duty/2; //50% duty
    OC2RS = 4500; //90%
    ...
    switch (appData.state) {
        ...
        case APP_STATE_CHECK_TIMER:
            ...
            if (gotRx || _CP0_GET_COUNT() - startTime > (48000000 / 2 / 5)) {
                ...
                if(rxVal >= 320){
                    OC1RS = duty;
                    OC3RS = (int)(0.9*duty*(1 - (double)((rxVal - 320)/320.0)));
                    OC2RS = 3000;
                }
                else{
                    OC3RS = duty;
                    OC1RS = (int)(0.9*duty*((double)(rxVal/320.0)));
                    OC2RS = 6000;
                }
            }
